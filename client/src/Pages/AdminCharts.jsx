      ...
      {charts.map((chart) => {
          ...
          datasets: [
            {
              ...
              data: values,
              // chart.js default colors will be used (we don't set colors here)
            },
          ],
        };
        ...
        return (
          <div key={chart._id} className="bg-white rounded-lg p-4 shadow relative">
            ...
            <div className="mb-2">
              {chart.type === "pie" ? <Pie data={data} /> : <Bar data={data} />}
            </div>

            <p className="text-sm text-gray-500">Uploaded by: <strong>{chart.createdBy?.username ?? "—"}</strong></p>
